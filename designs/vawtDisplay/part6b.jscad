...
function getParameterDefinitions() {
    return [
        ...
        },
        {
            ...
            caption: 'F. Lower gear 1 Tooth Count :',
            //(n1 > 0: external gear; n1 = 0: rack; n1 < 0: internal gear)
            type: 'int',
            initial: 23
        },
        
        {
            ...
            caption: 'G. Lower gear 2 Tooth Count:',
            type: 'int',
            initial: 11
        },
        {
            name: 'Uwheel1ToothCount',
            caption: 'H. Upper gear 1(platform) Tooth Count :',
            //(n1 > 0: external gear; n1 = 0: rack; n1 < 0: internal gear)
            type: 'int',
            initial: 79
        },
        ...
        {
            name: 'Uwheel2ToothCount',
            caption: 'I. Upper gear 2(pinion) Tooth Count:',
            type: 'int',
            initial: 11
        },
...
function main(params) {
    ...
    //calculate platform pinion gear distance to the center( achieved by circular pitch)
    var circularPitchRestored = params.gearRingDiameter * Math.PI / (Math.abs(params.Uwheel1ToothCount) + Math.abs(params.Uwheel2ToothCount));
    
    let diametralPitch = Math.PI / circularPitchRestored;
    let pitchDiameter1 =params.Uwheel1ToothCount / diametralPitch;
    let pitchDiameter2 =params.Uwheel2ToothCount / diametralPitch;
    //retrieve the center distance!
    let distance = (pitchDiameter1 - pitchDiameter2 )/2;

    //Recalculate the circular pitch for Lower gear set
    //Equation : Diametral pitch = (ToothCount2* ( ToothCount1/ToothCount2)+1) / ( 2* center distance)
    let DPitch = params.wheel2ToothCount*( (params.wheel1ToothCount/params.wheel2ToothCount)+1  ) / (2 * distance)
    //Equation: circular pitch = PI / Diametral pitch
    var circularPitch = Math.PI / DPitch;
    ...
    //get the circular pitch from gear distance!!
    //var centerDistance = distance;
    //var circularPitchRestored = centerDistance *2 * Math.PI / (params.wheel1ToothCount + params.wheel2ToothCount)
    var gear1 = new Gear({
...
var Gear = (function () {
    function Gear(options) {
        ...
        //Determine gear type used, give value if there's none
    ...
    Gear.prototype._createRegularGearShape = function () {
        ...
        /* cut hole after helix rotation(otherwise the hole will be twisted when making helical and herring bone gears)
...
var GearSet = (function () {
    ...
    GearSet.prototype.createShape = function ( ) {
        ...
        */
        ...
        /* Disabled fearture: twist angle for herringbone & helical gears
        let helical_angle = twist;

        //calculate twist angle of each gear to make them into helical/herringbone gear
        let module1 = this.gear1.outerRadius  *2 / this.gear1.toothCount;
        let module2 = this.gear2.outerRadius *2 / this.gear2.toothCount;
       
        let twist1 =(360 * gear_height * Math.tan ( Math.PI * helical_angle / 180 )  ) / (module1 * this.gear1.toothCount * Math.PI);
        
        let twist2 =-(360 * gear_height * Math.tan ( Math.PI * helical_angle / 180 )  ) / (module2 * this.gear2.toothCount * Math.PI);
        */
        
        var gear1CSG,gear2CSG;
        if (this.showOption ==1){
            gear1CSG = linear_extrude( {height : gear_height},gear1Shape);
        }else if(this.showOption ==2){
            gear2CSG = linear_extrude( {height : gear_height},this.gear2.getZeroedShape());
        ...
        if (this.showOption ==1){
            return  gear1CSG.union(cylinder({start: [0,0,gear_height], end: [0,0,gear_height+2.5], r: 7, fn:32})).subtract(cylinder({start: [0,0,0.8], end: [0,0,gear_height + 10], r: 4.1, fn:6})).subtract(cylinder({r: 1.45,start: [0,0,2], end: [0,0,-3], fn:32})) 
        }else if (this.showOption == 2) {
            //return gear2CSG.union(gear2CSG.mirroredZ()).union(cylinder({r: 4.8,start: [this.gearsDistance,0,0], end: [this.gearsDistance,0,-10], fn:6}).subtract(cylinder({r: 4.2,start: [this.gearsDistance,0,0], end: [this.gearsDistance,0,-10], fn:6})))
            return gear2CSG.subtract( cylinder({start: [this.gearsDistance,0,0], end: [this.gearsDistance,0,7], r1: 4.2, r2:4.2, fn:6})).union( cylinder({start: [this.gearsDistance,0,9], end: [this.gearsDistance,0,12], r1: 5, r2: 3, fn: 6}) ).union( cylinder({start: [this.gearsDistance,0,12], end: [this.gearsDistance,0,17], r1: 2.5, r2: 0, fn: 64}) )
        }else{
            return [ gear1CSG.union(cylinder({start: [0,0,gear_height], end: [0,0,gear_height+2.5], r: 7, fn:32})).subtract(cylinder({start: [0,0,0.8], end: [0,0,gear_height + 10], r: 4.1, fn:6})).subtract(cylinder({r: 1.45,start: [0,0,2], end: [0,0,-3], fn:32})) ,
                     gear2CSG.subtract( cylinder({start: [this.gearsDistance,0,0], end: [this.gearsDistance,0,7], r1: 4.2, r2:4.2, fn:6})).union( cylinder({start: [this.gearsDistance,0,9], end: [this.gearsDistance,0,12], r1: 5, r2: 3, fn: 6}) ).union( cylinder({start: [this.gearsDistance,0,12], end: [this.gearsDistance,0,17], r1: 2.5, r2: 0, fn: 64}) )];//
        }
