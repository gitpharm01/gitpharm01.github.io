...
function main(params) {
    ...
    var gearRingDiameter = params.gearRingDiameter;
    //get the circular pitch from gear distance!!
    var circularPitchRestored = gearRingDiameter * Math.PI / (Math.abs(params.wheel1ToothCount) + Math.abs(params.wheel2ToothCount));
    
    
    var gear1 = new Gear({
        circularPitch: circularPitchRestored,
        pressureAngle: params.pressureAngle,
        clearance: params.clearance,
        backlash: params.backlash,
        toothCount: Math.abs( params.wheel1ToothCount ) *-1, 
...
function getParameterDefinitions() {
    return [
        ...
        /* One shaft size only: 8mm in diameter, plus 0.4mm for hole{
            name: 'wheel1CenterHoleDiamater',
            caption: 'G. Wheel 1 Center Hole Diameter :',
            //(0 for no hole)
            type: 'float',
            initial: 4
        }, */
        {
            ...
            caption: 'H. Wheel 2 Tooth Count:',
            type: 'int',
            initial: 11
        },
        /* One shaft size only: 8mm in diameter, plus 0.4mm for hole{
            name: 'wheel2CenterHoleDiamater',
            caption: 'I. Wheel 2 Center Hole Diameter (0 for no hole):',
            type: 'float',
            initial: 4
        },*/
        {
...
var GearSet = (function () {
    ...
    GearSet.prototype.createShape = function () {
        ...
        if (this.showOption ==1){
            gear1CSG = linear_extrude( {height : gear_height+2.8},gear1Shape);
         ...
         if (this.showOption ==1){
            return gear1CSG.union(platform).union(cylinder({start: [0,0,0], end: [0,0,13.8], r: 7, fn:32} ) ).subtract(cylinder({start: [0,0,2.8], end: [0,0,13.8], r: 4.2, fn:32} ) )
        }else if (this.showOption == 2) {
            ...
            return gear2CSG.translate([0,0,2.8]).union( cylinder({start: [this.gearsDistance,0,7], end: [this.gearsDistance,0,11.8], r1: 4, r2:5.5, fn: 32}) ).union( cylinder({start: [this.gearsDistance,0,11.8], end: [this.gearsDistance,0,21.8], r: 4.05, fn: 32}) ).union( cylinder({start: [this.gearsDistance,0,21.8], end: [this.gearsDistance,0,32.8], r: 4, fn: 6}) ).subtract(  cylinder({start: [this.gearsDistance,0,20.8], end: [this.gearsDistance,0,32.8], r: 1.45, fn: 32}) )
        }else{
            return [gear1CSG.union(platform).union(cylinder({start: [0,0,0], end: [0,0,13.8], r: 7, fn:32} ) ).subtract(cylinder({start: [0,0,2.8], end: [0,0,13.8], r: 4.2, fn:32} ) ),
                 gear2CSG.translate([0,0,2.8]).union( cylinder({start: [this.gearsDistance,0,7], end: [this.gearsDistance,0,11.8], r1: 4, r2:5.5, fn: 32}) ).union( cylinder({start: [this.gearsDistance,0,11.8], end: [this.gearsDistance,0,21.8], r: 4.05, fn: 32}) ).union( cylinder({start: [this.gearsDistance,0,21.8], end: [this.gearsDistance,0,32.8], r: 4, fn: 6}) ).subtract(  cylinder({start: [this.gearsDistance,0,20.8], end: [this.gearsDistance,0,32.8], r: 1.45, fn: 32}) )];//
        }
